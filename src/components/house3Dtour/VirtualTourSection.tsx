...
const VirtualSectionImagesGroup = ({
  ...
  return (
    <div className={style.imgGroup}>
      ...
      <div className={style.imgContainer}>
        <a href={casa3D2Link} target='_blank' rel='noopener'>
          <img
            src={`${driveUrl}${casa3D2Imagen}`}
            className={style.imgResponsive}
            alt='Recorrido casa 2'
          />
        </a>
      </div>
    </div>
...
const VirtualHouseTourSection: React.FC<CommonProps> = ({ data, driveUrl }) => {
  return (
    <section id='VirtualTour' className={`${style.sectionContainer}`}>
      <div
        className={`${commonStyle.sectionTitle} text-center pl-40 pr-40 mb-80 fadeInDown animated`}
      >
        <span style={{ color: `var(--main-color)` }}>
          {data.casaVirtualTitleSection}
        </span>
      </div>
      {/* <div
      ...
      <div>
        <VirtualSectionImagesGroup
          ...
          casa3D2Link={data.casa3D2Link}
          casa3D2Imagen={data.casa3D2Imagen}
          driveUrl={driveUrl}
        />
      </div>
      {/* </div> */}
